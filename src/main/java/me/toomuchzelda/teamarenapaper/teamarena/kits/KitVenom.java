...
public class KitVenom extends Kit {
	...
	public KitVenom() {
		...
		setArmor(
			...
			ItemBuilder.of(Material.GOLDEN_CHESTPLATE)
				.trim(TrimMaterial.EMERALD, TrimPattern.RIB)
				.build(),
			ItemBuilder.of(Material.IRON_LEGGINGS)
				.trim(TrimMaterial.EMERALD, TrimPattern.RIB)
				.build(),
			new ItemStack(Material.IRON_BOOTS)
		);
		ItemStack sword = ItemBuilder.of(Material.IRON_SWORD)
			.name(Component.text("Poison Spear"))
			.setData(DataComponentTypes.ITEM_MODEL, Material.TIPPED_ARROW.key())
			.setData(DataComponentTypes.POTION_CONTENTS, PotionContents.potionContents().potion(PotionType.POISON).build())
			.setData(DataComponentTypes.TOOLTIP_DISPLAY, TooltipDisplay.tooltipDisplay().addHiddenComponents(DataComponentTypes.POTION_CONTENTS).build())
			.lore(
				Component.text("Poison I", TextColor.color(170, 170, 170)),
				Component.text("Lunge ?", TextColor.color(170, 170, 170)),
				Component.empty(),
				Component.text("Right click to leap forward, infecting everyone you touch!", TextColors.RIGHT_CLICK_TO)
			)
	...
	public static class VenomAbility extends Ability
	{
		...
		public static final int LEAP_CD = 10 * 20;
		public static final int LEAP_CD_DECREMENT = 3 * 20;
		...
		public void applyPoison(LivingEntity victim, Player giver){
			...
			if(victim.hasPotionEffect(PotionEffectType.POISON)){
				POISONED_ENTITIES.put(victim, victim.getPotionEffect(PotionEffectType.POISON).getDuration());
		...
		public void onInteract(PlayerInteractEvent event) {
			...
			if(event.getMaterial() == Material.IRON_SWORD) {
				...
				else if(!player.hasCooldown(Material.IRON_SWORD)) {
					//Leap Ability Implementation
					World world = player.getWorld();
					Vector newVelocity = player.getVelocity();
					Vector direction = player.getLocation().getDirection();
					double groundCompensation = player.isOnGround() ? 1.5d : 0.9d; // compensate non-jumping leaps
					newVelocity.setX(newVelocity.getX() * 0.5 + direction.getX() * groundCompensation);
					newVelocity.setY(newVelocity.getY() * 0.5 + direction.getY() * groundCompensation * 0.75 + 0.2);
					newVelocity.setZ(newVelocity.getZ() * 0.5 + direction.getZ() * groundCompensation);
					player.setCooldown(Material.IRON_SWORD, LEAP_CD);
					//world.playSound to play sound for all players
					world.playSound(player, Sound.ENTITY_WITHER_SHOOT, 0.3f, 1.1f);
					EntityUtils.setVelocity(player, newVelocity);
				...
				if (activeDuration <= 0 || Main.getGame().getGameState() != GameState.LIVE) {
					...
					for (Entity entity : nearby) {
						if (!Main.getGame().isDead(entity) && entity instanceof LivingEntity victim &&
							!leapVictims.contains(entity) && !(entity instanceof ArmorStand)) {
							//Applying DMG + Sounds
                            int newCooldown = player.getCooldown(Material.IRON_SWORD);
							...
							player.setCooldown(Material.IRON_SWORD, newCooldown);
